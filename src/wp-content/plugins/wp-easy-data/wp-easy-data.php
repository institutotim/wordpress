...
function Wp_easy_data_init() {
	...
	class Wp_easy_data {
		...
		*/
		 ...
		 */
	   public function Wp_easy_data ($model, $file, $permission = 'manage_options') {
	        global $wpdb;
            ...
            $this->external_name = isset($model['external_name']) && $model['external_name'] ? $model['external_name'] : $this->name . '_ID';
            $this->external_field = isset($model['external_field']) && $model['external_field'] ? $model['external_field'] : 'ID';
			
			
	        $this->relative_path = str_replace(WP_CONTENT_DIR . '/plugins/', '', $this->file);
		
	        $this->sortable = isset($model['sortable']) && $model['sortable'] ? true : false;
	        $this->parent_menu = isset($model['parent_menu']) && $model['parent_menu'] ? $model['parent_menu'] : false;
            $this->topMenuName = isset($model['topMenuName']) && $model['topMenuName'] ? $model['topMenuName'] : false;
	        $this->external_label_index = isset($model['external_label_index']) && $model['external_label_index'] ? $model['external_label_index'] : 0;
	        $this->custom_external_label_index = isset($model['custom_external_label_index']) && $model['custom_external_label_index'] ? $model['custom_external_label_index'] : array();
			...
			if (isset($_GET['page']) && quotemeta($_GET['page']) !== FALSE)
				$this->is_admin = ((preg_match('|' . quotemeta($_GET['page']) . '|', $this->relative_path) && !$this->parent_menu)  || $_GET['page'] == $this->menu_name) ? true : false;
	        
			// Pagination
	        if (isset($_GET['pagination_current']) && $_GET['pagination_current']) $this->pagination_current = $_GET['pagination_current'];
	        if (isset($_GET['pagination_items_per_page']) && $_GET['pagination_items_per_page']) $this->pagination_items_per_page = $_GET['pagination_items_per_page'];
            
            $search = (isset($_GET['search'])) ? $_GET['search'] : '';
	        $this->pagination_items = $this->get_number_of_items($this->build_search_clause($search));
	   ...
	   public function save_relations($item) {
		...
		public function add_belongsTo( &$obj, $realationship_label, $realationship_description, $list_display = false, $form_position = 0 ) {
			...
			if (is_object($obj) && isset($obj->name) && isset($obj->table)) {
                           	if(!isset($this->belongsTo[$obj->name]))
                                    $this->belongsTo[$obj->name] = new stdClass();
		...
		function add_hasManyAndBelongsTo( &$obj, $realationship_label, $realationship_description, $list_display = false, $form_position = 0 ) {
			...
			if (is_object($obj) && isset($obj->name) && isset($obj->table) ) {
				if(!isset($this->hasManyAndBelongsTo[$obj->name]))
                                    $this->hasManyAndBelongsTo[$obj->name] = new stdClass();
	...
	class Wp_easy_dataItem {
	    ...
	    function Wp_easy_dataItem(&$crudObj, $id = false, $info = false ) {
	        ...
	        if(!isset($recursive_limit))
                    $recursive_limit = new stdClass();
